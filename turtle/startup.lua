-- // SRTO (Sectly's Remote Turtle Orchestrator) - Turtle Client

local args = {...}
local serverBase = args[1]

if not serverBase or serverBase == "" then
  print("ERROR: No server URL provided")
  print("Usage: srto <server_url>")
  return
end

if string.sub(serverBase, -1) == "/" then
  serverBase = string.sub(serverBase, 1, -2)
end

local connectionURL = serverBase .. ":5757"
